...
import ExtentionModal from "../../../components/Modal/ExtentionModal";
import ExtentionUpdate from "../../../components/Modal/Update/ExtentionUpdate";
import ProjectionDetails from "../../../components/Modal/ProjectionDetailsModal";

const ExtentionTable = () => {
    ...
    const statusOptions = [{value: true , label: 'Pending PO'},{value: false, label: 'PO Recieved'}];
    ...
    const fetchData = async () => {
        try {
            const res = await axios.get(`${apiUrl}/extention/getExtention`, {
            ...
            if (responseData.success === false) {
                ...
                if (query) {
                    filteredData = filteredData.filter((item) => {
                        const extentionDescriptionExists = item.description;
                        return (
                            (extentionDescriptionExists && item.description.toLowerCase().includes(query.toLowerCase()))
                        );
                    });
                }
                ...
                if (selectedLikeliness.length > 0) {
                    filteredData = filteredData.filter((item) =>
                        selectedLikeliness.map(likeliness => likeliness.value).includes(item.likeliness)
                    );
                }
    ...
    const handleInvoiceClose = () => {
        setIsProjectionDetailsOpen(false);
      };

     const handleLikelinessChange = (selectedoptions) => {
        setSelectedLikeliness(selectedoptions);
    }
    ...
    return (
        <div className="bg-white rounded-lg shadow-md p-6">
            <div className="flex justify-between items-center w-full">
                <p className="text-3xl font-bold text-blue-800 mb-4 ">Extentions</p>
                    ...
                    currentUser.user.user_type === "Admin") && (
                        <div>
                            <button
                                ...
                                type="button"
                                onClick={() => setExtentionModal(true)}
                            >
                                Add Extention
                            </button>
                            <button
            ...
            <div className="relative overflow-x-auto shadow-md sm:rounded-lg bg-white">
                <div className="flex items-center justify-between flex-column md:flex-row flex-wrap space-y-4 md:space-y-0 py-4 mx-5 bg-white dark:bg-gray-900">
                    ...
                    <div className="flex items-center gap-4">
                        <Select
                            id="selectedLikeliness"
                            value={selectedLikeliness}
                            onChange={handleLikelinessChange}
                            options={LikelinessOptions.map((likeliness) => ({
                                value: likeliness,
                                label: likeliness
                            }))}
                            isMulti
                                ...
                                currentItems.map((item) => (
                                    ...
                                    >
                                        <td className="px-6 py-4">
                                            {item.description || item?.projectExtention.project_title || 'NA'}
                                        </td>
                                        <td className="px-6 py-4">
                                            {item?.assignedProjectLeader.username || ''}
                                        </td>
                                        <td className="px-6 py-4">{item.extentionStartDate}</td>
                                        <td className="px-6 py-4">
                                            {item.extentionEndDate || "Not specified"}
                                        </td>
                                        <td className="px-6 py-4">
                                            {calculateMissionDuration(
                                                item.extentionStartDate,
                                                item.extentionEndDate
                                            ) + " Months" || "Not specified"}
                                        </td>
                                        <td className="px-6 py-4">
                                            {item.revenueProjection || "NA"} {getCurrencySymbol(item.currencyCode)}
                                        </td>
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            {(item.status ? 'Pending PO' : 'PO Recieved') || 'NA'}
                                        </td>
                                        <td className="px-6 py-4">
                                        ...
                                        <td className="text-sm text-gray-500 whitespace-nowrap text-center rounded-xl flex justify-center items-center">
                                            <span
                                                className={` badge  border ${item.likeliness === "High"
                                                    ? "badge bg-success/20 text-success rounded-full hover:top-0"
                                                    : item.likeliness === "Medium" ? "badge bg-warning/20 text-warning rounded-full hover:top-0" : "badge bg-danger/20 text-danger rounded-full hover:top-0"}`}>
                                                {item.likeliness}
                                            </span>
                                        </td>
                                        {(currentUser.user.user_type === "Leader" ||
            ...
            {extentionModal && (
                <ExtentionModal isOpen={extentionModal} onClose={handleExtentionModal} />
            )}
