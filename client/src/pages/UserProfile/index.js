import axios from 'axios';
import { useParams } from "react-router-dom";
import React, { useEffect, useState } from 'react'
import { useSelector } from "react-redux";
import { Link } from 'react-router-dom';

const UserProfile = () => {
    const [profileData, setProfileData] = useState({});
    const [profileImage, setProfileImage] = useState(null);
    const [missions, setMissions] = useState([]);
    const [userMissions, setUserMissions] = useState([]);
    const apiUrl = process.env.REACT_APP_API_URL;
    const { currentUser } = useSelector((state) => state.user);
    const { id } = useParams();
    // console.log(id)
    // console.log(typeof(id));

    // console.log(missions)
    // Convert buffer to base64
    const bufferToBase64 = (buffer) => {
        let binary = '';
        const bytes = new Uint8Array(buffer);
        bytes.forEach((byte) => (binary += String.fromCharCode(byte)));
        return window.btoa(binary);
    };

    useEffect(() => {
        if (profileData && profileData.profileImage && profileData.profileImage.data) {
            const base64String = bufferToBase64(profileData.profileImage.data);
            setProfileImage(`data:image/jpeg;base64,${base64String}`);
        }
    }, [profileData]);

    useEffect(() => {
        const getProfileData = async () => {
            try {
                const res = await axios.get(
                    `${apiUrl}/profile/getSingleProfileDetails/${id}`,
                    { withCredentials: true }
                );
                const profileData = res.data;
                if (profileData.success === false) {
                    console.log(profileData.message);
                } else {
                    setProfileData(profileData);
                    setMissions(profileData.missions)
                }
            } catch (error) {
                console.error(error.message);
            }
        };
        getProfileData();
    }, [apiUrl, id]);

    const fetchMissions = async () => {
        try {
            const response = await axios.get(`${apiUrl}/mission/users/${id}`);
            setUserMissions(response.data);
            console.log(response.data)
        } catch (error) {
            console.error("Error fetching missions:", error);
        }
    };
    useEffect(() => {
        fetchMissions();
    }, [apiUrl])

    return (
        <div className="pt-5">
            <div className="grid grid-cols-1 lg:grid-cols-3 xl:grid-cols-4 gap-5 mb-5">
                <div className="panel">
                    <div className="flex items-center justify-between mb-5">
                        <h5 className="font-semibold text-lg dark:text-white-light">Profile</h5>
                        {currentUser && currentUser.user.user_id === Number(id) && (
                            <Link to="/profile" className="ml-auto btn btn-primary p-2 rounded-full">
                                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-5 h-5">
                                    <path opacity="0.5" d="M4 22H20" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" />
                                    <path
                                        d="M14.6296 2.92142L13.8881 3.66293L7.07106 10.4799C6.60933 10.9416 6.37846 11.1725 6.17992 11.4271C5.94571 11.7273 5.74491 12.0522 5.58107 12.396C5.44219 12.6874 5.33894 12.9972 5.13245 13.6167L4.25745 16.2417L4.04356 16.8833C3.94194 17.1882 4.02128 17.5243 4.2485 17.7515C4.47573 17.9787 4.81182 18.0581 5.11667 17.9564L5.75834 17.7426L8.38334 16.8675L8.3834 16.8675C9.00284 16.6611 9.31256 16.5578 9.60398 16.4189C9.94775 16.2551 10.2727 16.0543 10.5729 15.8201C10.8275 15.6215 11.0583 15.3907 11.5201 14.929L11.5201 14.9289L18.3371 8.11195L19.0786 7.37044C20.3071 6.14188 20.3071 4.14999 19.0786 2.92142C17.85 1.69286 15.8581 1.69286 14.6296 2.92142Z"
                                        stroke="currentColor"
                                        strokeWidth="1.5"
                                    />
                                    <path
                                        opacity="0.5"
                                        d="M13.8879 3.66406C13.8879 3.66406 13.9806 5.23976 15.3709 6.63008C16.7613 8.0204 18.337 8.11308 18.337 8.11308M5.75821 17.7437L4.25732 16.2428"
                                        stroke="currentColor"
                                        strokeWidth="1.5"
                                    />
                                </svg>
                            </Link>
                        )}
                    </div>
                    <div className="mb-5">
                        <div className="flex flex-col justify-center items-center">
                            <img src={profileImage ? profileImage : './userProfile.png'} alt="img" className="w-24 h-24 rounded-full object-cover  mb-5" />
                            <p className="font-semibold text-primary text-xl">{profileData.username}</p>
                        </div>
                        <ul className="mt-5 flex flex-col max-w-[300px] m-auto space-y-4 font-semibold text-white-dark">
                            <li className="flex items-center gap-2">
                                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-5 h-5 shrink-0">
                                    <path
                                        d="M2.3153 12.6978C2.26536 12.2706 2.2404 12.057 2.2509 11.8809C2.30599 10.9577 2.98677 10.1928 3.89725 10.0309C4.07094 10 4.286 10 4.71612 10H15.2838C15.7139 10 15.929 10 16.1027 10.0309C17.0132 10.1928 17.694 10.9577 17.749 11.8809C17.7595 12.057 17.7346 12.2706 17.6846 12.6978L17.284 16.1258C17.1031 17.6729 16.2764 19.0714 15.0081 19.9757C14.0736 20.6419 12.9546 21 11.8069 21H8.19303C7.04537 21 5.9263 20.6419 4.99182 19.9757C3.72352 19.0714 2.89681 17.6729 2.71598 16.1258L2.3153 12.6978Z"
                                        stroke="currentColor"
                                        strokeWidth="1.5"
                                    />
                                    <path opacity="0.5" d="M17 17H19C20.6569 17 22 15.6569 22 14C22 12.3431 20.6569 11 19 11H17.5" stroke="currentColor" strokeWidth="1.5" />
                                    <path
                                        opacity="0.5"
                                        d="M10.0002 2C9.44787 2.55228 9.44787 3.44772 10.0002 4C10.5524 4.55228 10.5524 5.44772 10.0002 6"
                                        stroke="currentColor"
                                        strokeWidth="1.5"
                                        strokeLinecap="round"
                                        strokeLinejoin="round"
                                    />
                                    <path
                                        d="M4.99994 7.5L5.11605 7.38388C5.62322 6.87671 5.68028 6.0738 5.24994 5.5C4.81959 4.9262 4.87665 4.12329 5.38382 3.61612L5.49994 3.5"
                                        stroke="currentColor"
                                        strokeWidth="1.5"
                                        strokeLinecap="round"
                                        strokeLinejoin="round"
                                    />
                                    <path
                                        d="M14.4999 7.5L14.6161 7.38388C15.1232 6.87671 15.1803 6.0738 14.7499 5.5C14.3196 4.9262 14.3767 4.12329 14.8838 3.61612L14.9999 3.5"
                                        stroke="currentColor"
                                        strokeWidth="1.5"
                                        strokeLinecap="round"
                                        strokeLinejoin="round"
                                    />
                                </svg>{' '}
                                {profileData.designation}
                            </li>
                            <li className="flex items-center gap-2">
                                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-5 h-5 shrink-0">
                                    <path
                                        opacity="0.5"
                                        d="M5 8.51464C5 4.9167 8.13401 2 12 2C15.866 2 19 4.9167 19 8.51464C19 12.0844 16.7658 16.2499 13.2801 17.7396C12.4675 18.0868 11.5325 18.0868 10.7199 17.7396C7.23416 16.2499 5 12.0844 5 8.51464Z"
                                        stroke="currentColor"
                                        strokeWidth="1.5"
                                    />
                                    <path
                                        d="M14 9C14 10.1046 13.1046 11 12 11C10.8954 11 10 10.1046 10 9C10 7.89543 10.8954 7 12 7C13.1046 7 14 7.89543 14 9Z"
                                        stroke="currentColor"
                                        strokeWidth="1.5"
                                    />
                                    <path
                                        d="M20.9605 15.5C21.6259 16.1025 22 16.7816 22 17.5C22 19.9853 17.5228 22 12 22C6.47715 22 2 19.9853 2 17.5C2 16.7816 2.37412 16.1025 3.03947 15.5"
                                        stroke="currentColor"
                                        strokeWidth="1.5"
                                        strokeLinecap="round"
                                    />
                                </svg>
                                {profileData.location}
                            </li>
                            <li>
                                <button className="flex items-center gap-2 text-[13px]">
                                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className="shrink-0">
                                        <path
                                            opacity="0.5"
                                            d="M2 12C2 8.22876 2 6.34315 3.17157 5.17157C4.34315 4 6.22876 4 10 4H14C17.7712 4 19.6569 4 20.8284 5.17157C22 6.34315 22 8.22876 22 12C22 15.7712 22 17.6569 20.8284 18.8284C19.6569 20 17.7712 20 14 20H10C6.22876 20 4.34315 20 3.17157 18.8284C2 17.6569 2 15.7712 2 12Z"
                                            stroke="currentColor"
                                            strokeWidth="1.5"
                                        />
                                        <path
                                            d="M6 8L8.1589 9.79908C9.99553 11.3296 10.9139 12.0949 12 12.0949C13.0861 12.0949 14.0045 11.3296 15.8411 9.79908L18 8"
                                            stroke="currentColor"
                                            strokeWidth="1.5"
                                            strokeLinecap="round"
                                        />
                                    </svg>
                                    <span className="text-primary whitespace-normal">{profileData.email}</span>
                                </button>
                            </li>
                            <li className="flex items-center gap-2">
                                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path
                                        d="M5.00659 6.93309C5.04956 5.7996 5.70084 4.77423 6.53785 3.93723C7.9308 2.54428 10.1532 2.73144 11.0376 4.31617L11.6866 5.4791C12.2723 6.52858 12.0372 7.90533 11.1147 8.8278M17.067 18.9934C18.2004 18.9505 19.2258 18.2992 20.0628 17.4622C21.4558 16.0692 21.2686 13.8468 19.6839 12.9624L18.5209 12.3134C17.4715 11.7277 16.0947 11.9628 15.1722 12.8853"
                                        stroke="currentColor"
                                        strokeWidth="1.5"
                                    />
                                    <path
                                        opacity="0.5"
                                        d="M5.00655 6.93311C4.93421 8.84124 5.41713 12.0817 8.6677 15.3323C11.9183 18.5829 15.1588 19.0658 17.0669 18.9935M15.1722 12.8853C15.1722 12.8853 14.0532 14.0042 12.0245 11.9755C9.99578 9.94676 11.1147 8.82782 11.1147 8.82782"
                                        stroke="currentColor"
                                        strokeWidth="1.5"
                                    />
                                </svg>
                                <span className="whitespace-normal" dir="ltr">
                                    {profileData.contactCode + " "}
                                    {profileData.contactDetails}
                                </span>
                            </li>
                        </ul>

                    </div>
                </div>
                <div className="panel lg:col-span-2 xl:col-span-3">
                <div className="flex mb-5 justify-between">
                        <div>
                            <h5 className="font-semibold text-lg dark:text-white-light">Task</h5>
                        </div>

                        <div className="flex items-center gap-3">
                            {/* Progress bar */}
                            <div className="w-96 h-4 bg-[#ebedf2] dark:bg-dark/40 rounded-full">
                                <div
                                    className="bg-primary h-4 rounded-full animated-progress"
                                    style={{
                                        width: `${profileData.totalOccupancy !== null ? profileData.totalOccupancy : 0}%`,
                                        backgroundImage:
                                            'linear-gradient(45deg, hsla(0, 0%, 100%, 0.15) 25%, transparent 0, transparent 50%, hsla(0, 0%, 100%, 0.15) 0, hsla(0, 0%, 100%, 0.15) 75%, transparent 0, transparent)',
                                        backgroundSize: '1rem 1rem',
                                    }}
                                ></div>
                            </div>

                            {/* Total Occupancy Label */}
                            <div className="flex gap-2 mt-1">
                                <span className="font-semibold text-lg">Total Occupancy</span>
                                <span className="font-bold text-primary text-lg">{profileData.totalOccupancy == null ? 0 : profileData.totalOccupancy}%</span>
                            </div>
                        </div>
                    </div>
                    <div className="mb-5">
                        <div className="table-responsive text-[#515365] dark:text-white-light font-semibold">
                            <table className="whitespace-nowrap">
                                <thead>
                                    <tr>
                                        <th>Sr. No.</th>
                                        <th>Projects</th>
                                        <th>Progress</th>
                                        <th>occupancy</th>
                                        <th>Status</th>
                                        <th className="text-center">Time</th>

                                    </tr>
                                </thead>
                                <tbody className="dark:text-white-dark">
                                    {
                                        missions && missions.map((mission) => (
                                            <tr key={missions.indexOf(mission)}>
                                                <td className='text-center'>{missions.indexOf(mission) + 1}</td>
                                                <td className='text-center cursor-pointer'
                                                  onClick={() => window.open(`/mission-details/${mission.id}`)}
                                                >{mission.description}</td>
                                                <td>
                                                    <div className="h-1.5 bg-[#ebedf2] dark:bg-dark/40 rounded-full flex w-full">
                                                        <div
                                                            className={`${mission.status === "Yet to Start" ? " bg-info " :
                                                                mission.status === "In Progress" ? " bg-success" :
                                                                    mission.status === "Closed" ? " bg-black" :
                                                                        ""
                                                                } rounded-full`}
                                                            style={{ width: `${mission.individualOccupancy}%` }}
                                                        ></div>
                                                    </div>
                                                </td>
                                                <td className={`${mission.status === "Yet to Start" ? " text-info " :
                                                    mission.status === "In Progress" ? " text-success" :
                                                        mission.status === "Closed" ? " text-black" :
                                                            ""
                                                    } text-center`}>{mission.individualOccupancy}%</td>
                                                <td className="text-success text-center"><span
                                                    className={`badge border ${mission.status === "Yet to Start" ? "badge bg-info/20 text-info rounded-full hover:top-0" :
                                                        mission.status === "In Progress" ? "badge bg-success/20 text-success rounded-full hover:top-0" :
                                                            mission.status === "Closed" ? "badge bg-black/40 text-white rounded-full hover:top-0" :
                                                                ""
                                                        }`}>
                                                    {mission.status}
                                                </span></td>
                                                <td className="text-center">{mission.duration} months</td>
                                            

                                           

                                            </tr>
                                        ))
                                    }
                                </tbody>

                            </table>

                        </div>
                    </div>
                </div>
            </div>
        </div>
    )
}

export default UserProfile





